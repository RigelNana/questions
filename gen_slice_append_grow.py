...
data = {
    ...
    "content": (
        ...
        "如果你只背了"小于 1024 翻倍，大于 1024 增长 25%"这条规则，那你可能从来没有真正理解过 Go 的扩容策略。"
    ),
    "answer": (
    ...
    "quiz": [
        ...
        },
        {
            ...
            "answer": "C",
            "explanation": "C 正确：make([]int, 0, expectedSize) 一次性预分配了足够的容量，后续 append 在不超过 cap 时不会触发任何新的内存分配。A 错误：var s []int 声明的是 nil slice，cap 为 0，第一次 append 就要分配内存，之后每次扩容都需要重新分配。B 错误：[]int{} 同样 cap 为 0，和 nil slice 在 append 行为上没有区别。D 错误：new([]int) 分配的是一个指向 nil slice 的指针，不仅没有预分配容量，还多了一层不必要的间接引用。"
        },
        {
            "id": "go-slice-grow-q6",
            ...
            "answer": "D",
            "explanation": "D 正确：虽然公式计算出的最小 cap 是 1，但 mallocgc 会将实际分配大小向上取整到最近的 size class。例如 append 一个 int64（8字节）时，分配的最小 size class 是 8 字节，正好容纳 1 个元素所以 cap=1；但对于更小的类型（如 int8），最小 size class 8 字节能容纳 8 个元素，所以 cap=8。A 错误：nil slice 完全可以 append，这是 Go 的设计特性。B 错误：对于某些元素类型 cap 确实是 1，但这不是通用答案。C 错误：cap=2 只在特定元素大小下才会出现。"
        },
        {
            "id": "go-slice-grow-q7",
        ...
        },
        {
            ...
            "answer": "C",
            "explanation": "C 正确：对于包含指针的元素类型，growslice 会调用 memclrNoHeapPointers 将新空间清零，确保 GC 不会把未初始化的随机字节误认为合法指针。对于不含指针的类型则跳过此步骤以提升性能。A 错误：两者在零值填充上确实有区别，这个优化在大 slice 上可以产生 8%-12% 的性能差异。B 错误：初始 cap 的计算与元素是否包含指针无关。D 错误：所有类型的 slice 都可以动态扩容，这是 Go slice 的基本特性。"
        },
        {
